# Therapay — Marketing Window (CLAUDE.marketing.md)

## SCOPE: This window — marketing pages only

### Own:
- `app/(marketing)/` — all marketing routes
- `components/marketing/` — marketing-specific components (hero, feature grids, pricing cards, etc.)
- `public/` — images and static assets for marketing pages

### Do NOT touch:
- `app/(app)/` — authenticated dashboard (other window owns this)
- `app/login/`, `app/layout.tsx`, `app/globals.css`
- `app/api/`
- `components/` root-level files (excluding `components/marketing/`)
- `lib/`, `proxy.ts`, `package.json`, `tsconfig.json`, `next.config.ts`

---

## Marketing Route Structure

Route group `(marketing)` — the parens make it invisible in URLs:
...
Create `app/(marketing)/layout.tsx` with a marketing nav (logo + links to /pricing, /about, /blog + CTA to /login) and a footer.

## Dark Mode Note
The root layout (`app/layout.tsx`) has `dark` class on `<html>`. Marketing pages inherit this.
Options:
- Design dark intentionally (consistent with app brand)
- Override per-section with a `<div className="light ...">` wrapper in `(marketing)/layout.tsx`

## Marketing Components
- Create in `components/marketing/` — keep separate from app components
...
## Auth
- **Library:** Supabase Auth via `@supabase/ssr`
- **Login flow:** `supabase.auth.signInWithPassword({ email, password })` in the client
- **Session reading:** `createSupabaseServerClient()` from `lib/supabase-server.ts` + `supabase.auth.getUser()` in Server Components
- **Test accounts:** sarah@therapay.dev, marcus@therapay.dev, jordan@therapay.dev (all use `password123`)
- **Env vars:** `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_DEFAULT_KEY` in `.env.local`

## Proxy (Middleware)
- File is `proxy.ts` at project root (Next.js 16 convention)
- Export must be named `proxy` (not `middleware`)
- Public paths already include: `/`, `/pricing`, `/about`, `/blog`, `/login` — marketing routes need no auth

## Testing After Changes
```bash
npm run build
npm run dev > /tmp/therapay-marketing-dev.log 2>&1 &
cat /tmp/therapay-marketing-dev.log | tail -20
```
...
## Branch Info
- This worktree: `/Users/paulhoover/therapay-marketing` on branch `feat/marketing-pages`
- App worktree: `/Users/paulhoover/therapay` on branch `main`
- Do NOT run `git worktree` commands or switch branches
- This file (`CLAUDE.marketing.md`) merges into `main` and lives there permanently for future reuse
